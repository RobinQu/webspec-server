...
var github = function(conf) {
  ...
  Github.refs = {
    remove: function(options) {
      assert(options.ref, "should have ref");
      assert(options.repo, "should have repo");
      var url = [endpoint, "repos", options.repo, "git/refs", options.ref].join("/");
      return function(callback) {
        superagent.del(url).end(function(e, res) {
          if(e) {
            return callback(e);
          }
          if(res.ok) {
  ...
  Github.repo = {
    ...
    get: function(options) {
      assert(options.user, "should have username");
      assert(options.name, "should have repo name");
      var url = [endpoint, "repos", options.user, options.name].join("/");
      return function(callback) {
    ...
    commits: function(options) {
      assert(options.repo, "should have repo");
      var url = [endpoint, "repos", options.repo, "commits"].join("/");
      
      return function(callback) {
        debug("fetch commits of %s, %s", options.repo, options.path);
        superagent.get(url).query({path: options.path})
        .end(function(e, res) {
          callback(e, res.body);
        });
      };
    ...
    access: function(options) {
      ...
      return function(callback) {
        debug("access %s %s", options.repo, options.path);
        superagent.get(url)
